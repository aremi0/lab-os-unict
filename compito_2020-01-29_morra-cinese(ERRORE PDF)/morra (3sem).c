 ...
 *
 * N.B. forse con input molto grossi l'aver utilizzato 3 semafori puo'
 * produrre output minore di quanto ci si aspetterebbe (anche se non mi e' mai successo)
 */
...
void giudice(int nPartite, gameStatus *shm, int coda, int sem){
    ...
    for(int i = 1; i <= nPartite; i++){
        ...
        SIGNAL(sem, P_SEM); //...altrimenti risveglio i giocatori
        SIGNAL(sem, P_SEM);
    }
...
int main(int argc, char *argv[]){
    ...
    if(semctl(semId, P_SEM, SETVAL, 0) == -1){
        perror("semP set val");
        exit(1);
    }
